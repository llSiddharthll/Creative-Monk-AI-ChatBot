/*
  ...
  1. New Tables
    - `chat_sessions`
      - `id` (uuid, primary key)
      - `user_id` (text, for anonymous users we'll use browser fingerprint)
    ...
    - `chat_messages`
      ...
      - `content` (jsonb, supports both text and multimodal content)
      - `created_at` (timestamp)
...
-- Create policies for anonymous access (using user_id as identifier)
